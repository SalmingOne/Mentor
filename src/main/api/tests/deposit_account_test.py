...
class TestDepositAccount:
    ...
    def test_deposit_account(self, api_manager, deposit_account_request, db_session):
        ...
        assert deposit_account_request.account_response.balance + deposit_account_request.deposit_request.amount == response.balance

        transaction_from_db = Transaction.get_transaction_by_type(db_session, 'deposit')
        assert deposit_account_request.deposit_request.account_id == transaction_from_db.to_account_id
        assert deposit_account_request.deposit_request.amount == transaction_from_db.amount
